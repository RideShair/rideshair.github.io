function ticker(id) {
    ...
    var massiveItemCount, CurrentStory, CurrentLength, AnchorObject, StorySummary, list;

    function startTicker() {
        AnchorObject = document.getElementById(id);
        list = AnchorObject.innerHTML.split(' / ');
        massiveItemCount = Number(list.length);
        CurrentStory = -1;
        CurrentLength = 0;
        runTheTicker();
    }
    ...
    function runTheTicker() {
        ...
        if (CurrentLength === 0) {
            ...
            StorySummary = list[CurrentStory].replace(/"/g, '-');
        }
